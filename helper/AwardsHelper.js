...
async function upload(title, contents){
    ...
    if(file != null){
        const storageRef = ref(storage, `awards/${docRef.id}/0.png`);
        await uploadBytes(storageRef, file).then((snapshot) => {
            file = null;

            getDownloadURL(ref(storage, `awards/${docRef.id}/0.png`)).then(async (url) => {
                await updateDoc(doc(db, 'Awards', docRef.id), {"image": url}).then(function(){
...
async function modify(id, title, contents){
    ...
    if(file != null){
        const storageRef = ref(storage, `awards/${id}/0.png`);
        await uploadBytes(storageRef, file).then((snapshot) => {
            file = null;
...
async function remove(id){
    if (currentPost.url != null) {
        const imgRef = ref(storage, `awards/${id}/0.png`);
...
async function get(){
    ...
    querySnapshot.forEach((doc) => {
        ...
        datas.push(
            new AwardsDataModel(doc.id, title, contents, url, date)
        );
    })
